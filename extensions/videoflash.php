...
function wfVideoFlash() {
        global $wgParser;
        $wgParser->setHook('video', 'renderVideoFlash');
}
...
function renderVideoFlash($input, $args) {
        ...
        $type       = isset($args['type'],$url[$args['type']]) ? $args['type'] : 'html5';
        $media_url  = isset($url[$type]) ? $url[$type] : '';
		...
		$ns12 = substr( $id ,'0' ,'6'  );
            ...
            if (strtolower($ns12) == 'image:') 
			{ 
			$files22 = ucfirst(substr( $id ,'6' ));
			$path11 = hash('md5', $files22 );
			$path123 = substr( $path11 ,'0' ,'1' ).'/'.substr( $path11 ,'0' ,'2'  ).'/'.$files22;
			...
			$output = '<video controls';
                     ...
                     $output .= '><source src="/uploads/mediawiki/'.$path123.'"></video>';
                     return $output;
			}
			else {
