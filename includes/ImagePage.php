	...
	function openShowImage() {
		...
		if ( $this->img->exists() ) {
			...
			if ($showLink) {
				...
				if (!$this->img->isSafeFile()) {
				if (substr($full_url, -4, 1) == ".") $ext = substr($full_url, -3);
				else if (substr($full_url, -5, 1) == ".") $ext = substr($full_url, -4);
					switch ($ext)
	{
		case "mp4":
		case "webm":
...
<div class="media" id="file">
<video controls src="{$full_url}"></video>
</div><div class="fullMedia"><p>
<a href="$full_url">$filename</a>$dirmark 
<span class="fileInfo"> ($info)</span></p>
</div>
